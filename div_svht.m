function    x   =   div_svht(lambda, varargin)
    %
    ...
    if( is_real ),
        x   =   div_svht_real(lambda, s, idx_p, M, N);
    else
        x   =   div_svht_complex(lambda, s, idx_p, M, N);
    end
end
function    x   =   div_svht_real(lambda, s, idx_p, M, N)
    x  	=   0;
    if( any(idx_p)  ),
        x   =   x + sum( 0.5*s(idx_p, 2).*(s(idx_p, 2) + 1) );
        
        x   =   x + sum( (abs(M-N)*s(idx_p, 2) + 0.5*s(idx_p, 2).*(s(idx_p, 2) - 1)).*((s(idx_p,1).*(s(idx_p,1)>lambda))./s(idx_p, 1)) );
    end
    
    C  	=   zeros(size(s, 1));
    
    for Ik = 1:size(s, 1),
        C(:, Ik)    =   s(Ik, 2)*s(:, 2).*s(:, 1).*(s(:,1).*(s(:,1)>lambda))./(s(:, 1).^2 - s(Ik, 1).^2);
    end
    C( isnan(C) | isinf(C) | abs(C) > 1E6 )     =   0;
    
    x 	=   x + 2*sum(C(:));
end
function    x   =   div_svht_complex(lambda, s, idx_p, M, N)
    x  	=   0;
    if( any(idx_p)  ),
        x   =   x + sum( s(idx_p, 2).^2 );
