...
% y: measurements
% with best rank r low-rank matrix denoiser
function [L,S,errl,errs]=TMP(A,At,y,params)
% A: a partial orthogonal linear operator
% At: the transpose operator of A
% y: measurements
n1=params.n1;
...
for ii=1:iter
    ...
    %------------best rank-r low-rank estimator part-----------
    [U,sig,V]=lansvd(full(Lpri),r); Lpost=U*sig*V';
    noi=randn(n1,n2); Ln=Lpri+epsi*noi;
    [U1,sig1,V1] = lansvd(full(Ln),r); Ln=U1*sig1*V1';
    div=sum(sum((Ln-Lpost).*noi))/epsi/n; %div/n
    ext=Lpost-div*Lpri; c=sum(sum(ext.*(Lpri)))/norm(ext,'fro')^2;
    Lext=c*ext;
    ...
    if vl==1 % equ. (54c) asytopic
        ...
        if vl==2 % equ. (54b)
            ...
            vlext=(norm(Lpri,'fro')^2-sum(sum(ext.*Lpri))^2/norm(ext,'fro')^2)/n;
        end
    end
